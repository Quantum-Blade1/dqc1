...
public:
  static mlir::Operation *lowerEPRAlloc(mlir::Operation *op,
    ...
    // Map DQC attributes to MPI attributes
    if (auto srcAttr = op->getAttrOfType<mlir::IntegerAttr>("source_qpu"))
      adaptState.addAttribute("src_qpu", srcAttr);
    if (auto tgtAttr = op->getAttrOfType<mlir::IntegerAttr>("target_qpu"))
      adaptState.addAttribute("tgt_qpu", tgtAttr);
...
private:
  ...
  void lowerEPRAlloc(mlir::func::FuncOp func) {
    ...
    func.walk([&](mlir::Operation *op) {
      if (op->getName().getStringRef() == "dqc.epr_alloc")
        ops.push_back(op);
    });
    for (auto *op : ops) {
      rewriter.setInsertionPoint(op);
      DQCToMPILowering::lowerEPRAlloc(op, rewriter);
    }
  }
  ...
  void lowerTeleGate(mlir::func::FuncOp func) {
    ...
    llvm::SmallVector<mlir::Operation *, 16> ops;
    func.walk([&](mlir::Operation *op) {
      if (op->getName().getStringRef() == "dqc.telegate")
        ops.push_back(op);
    });
    for (auto *op : ops) {
      rewriter.setInsertionPoint(op);
      DQCToMPILowering::lowerTeleGate(op, rewriter);
    }
  }
