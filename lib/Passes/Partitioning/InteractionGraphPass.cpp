...
public:
  void runOnOperation() final {
    ...
    if (num_qubits == 0) num_qubits = 10; // Fallback
    ...
    func.walk([&](mlir::Operation *op) {
      // Look for multi-qubit operations (CNOT, CCX, Swap)
      if (op->getName().getStringRef().ends_with("cnot") ||
          op->getName().getStringRef().ends_with("ccx") ||
          op->getName().getStringRef().ends_with("swap")) {
        LLVM_DEBUG(llvm::dbgs() << "Found multi-qubit operation\n");

        // Extract qubit IDs from operation
        auto operands = op->getOperands();
        if (operands.size() >= 2) {
          llvm::SmallVector<int, 4> qubit_ids;
          for (auto operand : operands) {
            ...
            if (auto *def = operand.getDefiningOp()) {
              if (auto idAttr = def->getAttrOfType<mlir::IntegerAttr>("quir.qubit_id"))
                ...
                auto it = qubit_map.find(operand);
                if (it != qubit_map.end())
                  q_id = it->second;
              }
            }
            if (q_id == -1) {
              if (auto ba = llvm::dyn_cast<mlir::BlockArgument>(operand))
                q_id = static_cast<int>(ba.getArgNumber());
            }
            if (q_id != -1) {
              qubit_ids.push_back(q_id);
            }
          }

          if (qubit_ids.size() >= 2) {
            for (size_t i = 0; i < qubit_ids.size() - 1; ++i) {
              for (size_t j = i + 1; j < qubit_ids.size(); ++j) {
                hypergraph->addEdge(qubit_ids[i], qubit_ids[j]);
              }
            }
          }
        }
